...
Blocking: Main actor center stage, towards the audience, PC behind, maybe next to them.

One PC member represents the main actor in their story: PC1
...
_Character is led onto stage by PC2,3 as if in handcuffs, is being sat onto a chair (the only prop on stage). The character is then left alone on stage._
...
_PC encircle the main actor as demons escaping from hell, crawling, threatening, haunting._
...
_PC1 with gentleman-like gestures, assisting PC2 as the old man_
...
_Spotlight onto a PC member on a "bed", PC1 looking over_
...
_PC with haunting movements, in sync with the heartbeat sound_
...
_Spotlight, old man, bed, eyes wide open (maybe play with perspective and move the "bed" to a vertical position)_
...
_MC themselves does not move, but PC1 mirrors the words with movements._
